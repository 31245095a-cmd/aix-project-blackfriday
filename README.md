...
# 6.related work
https://www.kaggle.com/code/youcefbel/black-friday-sale-data-analysis-and-prediction딥러닝을 이용한 데이터 분석 외에 단순 통계를 통한 그래프는 다음 블로그를 참조하면 될 것이다.
현재 프로젝트를 하기 전 https://www.kaggle.com/datasets/noopurbhatt/retail-black-friday-sales-dataset에 있는 데이터셋을 사용하여 딥러닝을 통해 할인율, 나이, 주말 여부 등의 데이터를 이용 소비 매출 예측 등을 계획했었다. 그러나 해당 데이터는 가공된 데이터로 모든 데이터가 유의미한 값을 지니고 있다고 하기 어려웠다. 등급에 따른 매출액, 나이에 따른 매출액, 균등하게 분포된 할인율 등으로 인해 해당 데이터셋으로 상품 카테고리별 구매 확률 연령대별 구매 확률 등이 의미가 없어 다른 데이터셋을 찾기로 하였다
그 다음으로 https://www.kaggle.com/datasets/ibikunlegabriel/christmas-sales-and-trends에서 사용한 데이터셋을 사용해 분석하고자 했다. 해당 데이터셋은 11월,12월의 데이터가 크리스마스 기간, 블랙프라이데이 기간, 그 외의 기간이 포함되어 있었다. 해당 데이터에서 크리스마스 기간을 제외해 일반적인 기간과 블랙프라이 데이 기간을 비교분석하고자 했다. 해당 데이터셋에 나이, 성별, 구매 가격, 할인 가격, 판매일 등이 기재되어 있었다. <img width="1706" height="917" alt="image" src="https://github.com/user-attachments/assets/84907eb2-a4b9-4d65-a58a-4dd3b59ab340" /> 해당 데이터로는 카테고리 별 매출 분포, 건당 매출 평균액 등 유의미한 데이터 차이가 있었으나 실제 데이터셋을 보자 모든 상품의 할인율이 10%로 동일하였다. 이는 실제 데이터값과 차이가 있을 것이라 생각해 다른 데이터셋을 찾기로 하였다
